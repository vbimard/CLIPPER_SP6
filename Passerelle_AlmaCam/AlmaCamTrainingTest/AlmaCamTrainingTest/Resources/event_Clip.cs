...
");
            ...
            #region Formule pour le calcul personnalisé des propriétés d'une pièce

            {
                parameterValue = context.ParameterSetManager.GetParameterValue("_EVENT_FORMULA_HANDLER", "_QUOTE_PART_ASSISTANT_TYPE");
                parameterValue.SetValue(1);
                parameterValue.Save(transaction);
                parameterValue = context.ParameterSetManager.GetParameterValue("_EVENT_FORMULA_HANDLER", "_QUOTE_PART_ASSISTANT_FORMULA");
                parameterValue.SetValue(@"using System;
                using System.Diagnostics;
                using Wpm.Implement.Manager;
                using System.Windows.Forms;
                namespace Actcut.QuoteModelManager
                    {
                        public partial class QuotePartAssistant : QuotePartAssistantFormula
                        {
                            public override void CustomiseAssistant()
                            {       //recuperaiton du delais par defaut
       		 	                    string delais = Info.QuotePartEntity.Context.ParameterSetManager.GetParameterValue(""_CONFIGURATION"", ""_FABRICATION_PERIOD_TEXT"").GetValueAsString();
                                    // ecriture du delais
				                    Info.QuotePartEntity.SetFieldValue(""_FABRICATION_PERIOD_TEXT"", delais);
                    ...
                    ");
                parameterValue.Save(transaction);
                parameterValue = context.ParameterSetManager.GetParameterValue("_EVENT_FORMULA_HANDLER", "_QUOTE_PART_ASSISTANT_FILENAME");
                parameterValue.SetValue(null);
                parameterValue.Save(transaction);
                parameterValue = context.ParameterSetManager.GetParameterValue("_EVENT_FORMULA_HANDLER", "_QUOTE_PART_ASSISTANT_NAMESPACE");
                parameterValue.SetValue(null);
                parameterValue.Save(transaction);
                parameterValue = context.ParameterSetManager.GetParameterValue("_EVENT_FORMULA_HANDLER", "_QUOTE_PART_ASSISTANT_CLASS");
                parameterValue.SetValue(null);
                parameterValue.Save(transaction);
            }
